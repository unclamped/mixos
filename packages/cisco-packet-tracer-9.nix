...
# Vendored copy of nixpkgs' pkgs/by-name/ci/cisco-packet-tracer_9/package.nix,
# patched to add 9.0.1 support: the .desktop filenames inside the AppImage
# are version-stamped, but upstream's extraInstallCommands hardcoded the
# 9.0.0 literal. Drop this overlay once that fix lands upstream and the
# flake's nixpkgs input is bumped past it.
{
  appimageTools,
  dpkg,
  lib,
